...
function Search({ setError, resetError, setStudents, studentList, studentsFromDb  }) {
  ...
  const validateStudent = () => {
    ...
    const validStudent = studentsFromDb.filter(
      (std) =>
        currentStudent.name.toLowerCase() === std.name.toLocaleLowerCase()
    );
    ...
    if (!checkValidity(currentStudent.date, validStudent[0].validityDate)) {
      setError({
        status: true,
        message: `Sorry, ${currentStudent.name}'s validity has Expired!`,
      });
      return;
    }
